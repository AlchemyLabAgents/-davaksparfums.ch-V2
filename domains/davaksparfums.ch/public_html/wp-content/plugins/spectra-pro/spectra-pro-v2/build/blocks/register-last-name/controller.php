<?php
/**
 * Register Last Name Field controller.
 *
 * @since 2.0.0
 * @package Spectra Pro
 */
...
$field_type        = $attributes['fieldType'] ?? 'last_name'; // Field type for register-last-name block.
...
// Get context values for inheritance (following loop-builder pattern).
...
// Context label and placeholder values (following loop-builder pattern).
...
switch ( $field_type ) {
	case 'last_name':
		$effective_label       = $field_label ? $field_label : __( 'Last Name', 'spectra-pro' );
		$effective_placeholder = $field_placeholder ? $field_placeholder : __( 'Enter your last name', 'spectra-pro' );
...
$additional_classes = array(
	'spectra-pro-register-last-name',
	'spectra-pro-form-input-wrapper--' . esc_attr( $field_type ),
);

$wrapper_attributes = BlockAttributes::get_wrapper_attributes( $attributes, $config, array( 'id' => 'register-last-name-' . esc_attr( $block_id ) ), $additional_classes );
