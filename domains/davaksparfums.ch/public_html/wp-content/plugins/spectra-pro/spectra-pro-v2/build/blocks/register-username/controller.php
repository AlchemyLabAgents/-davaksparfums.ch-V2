<?php
/**
 * Register Username Field controller.
 *
 * @since 2.0.0
 * @package Spectra Pro
 */
...
$field_type  = $attributes['fieldType'] ?? 'username'; // Field type for register-username block.
...
switch ( $field_type ) {
	case 'username':
		$effective_label       = $field_label ? $field_label : __( 'Username or Email', 'spectra-pro' );
		$effective_placeholder = $field_placeholder ? $field_placeholder : __( 'Enter your username or email', 'spectra-pro' );
...
$additional_classes = array(
	'spectra-pro-register-username',
	'spectra-pro-form-input-wrapper--' . esc_attr( $field_type ),
);

$wrapper_attributes = BlockAttributes::get_wrapper_attributes( $attributes, $config, array( 'id' => 'register-username-' . esc_attr( $block_id ) ), $additional_classes );

// Provide context to child blocks using the correct context keys - FORCE fieldType context.
